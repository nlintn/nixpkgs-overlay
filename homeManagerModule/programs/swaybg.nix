  ...
  options.programs.swaybg = {
    ...
    image = {
      ...
      mode = lib.mkOption {
        ...
        type = lib.types.nullOr (lib.types.enum [ "stretch" "fit" "fill" "center" "tile" "solid_color" ]);
      };
    };
    color = lib.mkOption {
      default = null;
      ...
      type = lib.types.nullOr (lib.mkOptionType {
        ...
        check = (s: let
          ...
          in lib.types.str.check s && builtins.length s' == 6 && lib.all (x: lib.any (y: x == y) hex_chars) s');
      });
    };
  };
