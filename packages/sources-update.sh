...
new_json=$(nix-instantiate --option substitute false --eval --raw --argstr json "${prev_json}" --argstr nixpkgs "$NIXPKGS" "${BASE_PATH}/sources-update.nix" |
    jq --sort-keys .)
echo "${new_json}" >"${BASE_PATH}/sources.json"
